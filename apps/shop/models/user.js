...
schema.methods.addToCart = function(productId) {
    ...
    const productIndex = this.cart.items.findIndex((p => p.productData.toString() === productId.toString()));
...
schema.methods.removeFromCart = function (productId) {
    const updatedCartItems = this.cart.items.filter(item => item.productData.toString() !== productId.toString());
